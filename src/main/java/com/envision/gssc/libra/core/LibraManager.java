package com.envision.gssc.libra.core;

import com.envision.gssc.libra.batch.JobParametersProvider;
import com.envision.gssc.libra.core.metadata.JobMetadata;
import com.envision.gssc.libra.core.metadata.JobMetadataReader;
import com.envision.gssc.libra.core.metadata.StepMetadata;
import com.envision.gssc.libra.scheduling.JobTrigger;
import com.envision.gssc.libra.scheduling.JobTriggerDao;
 ...
 */
@Component
public class LibraManager implements StepExecutionListener, JobExecutionListener {
    ...
    public LibraManager(LibraConfig libraConfig, JobOperator jobOperator, JobRepository jobRepository, JobExplorer jobExplorer, JobRegistry jobRegistry) {
    ...
    public Long startJob(String jobName, String jobParams) throws JobParametersInvalidException, JobInstanceAlreadyExistsException, NoSuchJobException {
        ...
        if (env.containsProperty("libra.application.git.commit.id.abbrev")) {
            paramsStr.append(",revision=");
            paramsStr.append(env.getProperty("libra.application.git.commit.id.abbrev","?"));
            if (env.containsProperty("libra.application.git.total.commit.count")) {
                paramsStr.append("(").append(env.getProperty("libra.application.git.total.commit.count")).append(")");
            }
        }
        if (jobParams != null) {
    ...
    public SortedSet<String> listJobNames() {
        if (jobOperator.getJobNames().size() == 0) {
            ...
            TreeSet<String> metaJobNames = new TreeSet<>(jobMetadataReader.getJobMetadata().keySet());
            return metaJobNames;
        }
    ...
    public List<String> getServerInfo() {
        ...
        info.add("Libra version: " + env.getProperty("libra.version","---"));
        info.add("Libra build: " + env.getProperty("libra.build","---"));
        info.add("Libra build time: " + env.getProperty("libra.buildTime","---"));
        info.add("Application name: " + env.getProperty("libra.application.name","---"));
        info.add("Application description: " + env.getProperty("libra.application.description","---"));
        info.add("Application version: " + env.getProperty("libra.application.version","---"));
        info.add("Application build: " + env.getProperty("libra.application.build","---"));
        info.add("Application build time: " + env.getProperty("libra.application.buildTime","---"));
        info.add("GIT branch: " + env.getProperty("libra.application.git.branch","---"));
        info.add("Closest tag: " + env.getProperty("libra.application.git.closest.tag.name","---"));
        info.add("GIT commit count: " + env.getProperty("libra.application.git.total.commit.count","---"));
        info.add("GIT id: " + env.getProperty("libra.application.git.commit.id.abbrev","---"));
        info.add("GIT commit message: " + env.getProperty("libra.application.git.commit.message.short","---"));
        info.add("Home dir: " + env.getProperty("libra.homeDir","---"));
        info.add("Log dir: " + env.getProperty("libra.logDir","---"));
        info.add("Job dir: " + env.getProperty("libra.jobBeansDir","---"));
        info.add("Script dir: " + env.getProperty("libra.scriptDir","---"));
        info.add("Staging DB: " + env.getProperty("libra.stagingDB.url","---"));
        info.add("Staging DB user: " + env.getProperty("libra.stagingDB.username","---"));
    ...
    public String getShortServerInfo() {
        ...
        sb.append(" ").append(env.getProperty("libra.application.name","-"));
        sb.append(" ").append(env.getProperty("libra.application.buildTime",""));
        if (env.containsProperty("libra.application.git.commit.id.abbrev")) {
            sb.append(" ").append(env.getProperty("libra.application.git.branch"));
            sb.append(" ").append(env.getProperty("libra.application.git.commit.id.abbrev"));
            if (env.containsProperty("libra.application.git.total.commit.count")) {
                sb.append(" (").append(env.getProperty("libra.application.git.total.commit.count")).append(")");
            }
        }
        else {
            sb.append(" ").append(env.getProperty("libra.application.version"));
        }
        return  sb.toString();
    }
}
