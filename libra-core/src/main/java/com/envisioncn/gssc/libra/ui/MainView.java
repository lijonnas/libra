...
import com.envisioncn.gssc.libra.core.JobEventListener;
import com.envisioncn.gssc.libra.core.JobInstanceInfo;
...
public class MainView extends AppLayout implements JobEventListener {
    ...
    protected void onAttach(AttachEvent attachEvent) {
        ...
        try {
            serverInfoGrid.setItems(libraManager.getServerInfo());
            buildInfo.setText(libraManager.getShortServerInfo());
            //grid.setItems(libraManager.jobDetails());
            updateJobGrid(libraManager.jobDetails());
            filterJobGrid();
        } catch (Exception e) {
            log.warn("Failed to fetch job details", e);
            showErrorMessage("Failed to fetch job details");
        }
        libraManager.registerJobChangeListener(this);
    }
    ...
    private void updateJobGrid(List<JobInstanceInfo> jobs) {
        ...
        for (JobInstanceInfo job : jobs) {
            ...
            JobButtons jb = new JobButtons(job, this, libraManager);
    ...
    protected void onDetach(DetachEvent detachEvent) {
        ...
        libraManager.unregisterJobChangeListener(this);
        //grid.setItems(Collections.emptyList());
    }

    private void createMainView(SchedulingView schedulingView) {
        log.debug("createMainView");
        Image img = new Image("../static/images/libra-logo-light.png", "Libra logo");
        img.setHeight("28px");
    ...
    private InputStream createJobReport() throws Exception {
        ...
        List<JobInstanceInfo> jobDetails = libraManager.jobDetails();
    ...
    protected Component createJobHistory(String jobName) {
        ...
        try {
            jobs = libraManager.jobHistory(jobName);
        } catch (Exception e) {
